...
Read-only HTTP service over the same ensemble as CLI (no train/download in request path). Production may run **locally via Docker + Cloudflare Tunnel** ([deploy-local-tunnel.md](deploy-local-tunnel.md)) or on Fly.io ([deploy.md](deploy.md)).
